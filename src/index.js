...
const getJwtStrategy = (config) => {
  ...
  assert(
    ["aad", "secret"].includes(config.auth.type),
    "auth.type must be a know type (secret or aad)",
  );
  ...
  if (config.auth.type === "aad") {
    ...
    assert(
      config.auth.authorityHostUrl,
      "auth.authorityHostUrl configuration attribute required",
    );
    assert(
      config.auth.clientId,
      "auth.clientId configuration attribute required",
    );
    assert(
      config.auth.clientSecret,
      "auth.clientSecret configuration attribute required",
    );
    assert(
      config.auth.resource,
      "auth.resource configuration attribute required",
    );
